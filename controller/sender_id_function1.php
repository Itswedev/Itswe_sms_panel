...
else if($sender_type=='all_senderid_download')
{
    $table = SENDERID;
        ...
        //         );
           ...
           // $data = SSP::complex($_REQUEST, $sql_details, $table, $primaryKey, $columns, $test = null, $extraWhere);

            // $excelData = '';
            // foreach ($data as $row) {
            //     foreach ($row as $key => $value) {
            //         // Applying formatter
            //         if (isset($columns[$key]['formatter'])) {
            //             $value = $columns[$key]['formatter']($value, $row);
            //         }
            //         if (is_array($value)) {
            //             $excelData .= implode(',', $value) . "\t";
            //         } else {
            //             $excelData .= strip_tags($value) . "\t";
            //         }
            //     }
            //     $excelData .= "\n";
            // }
