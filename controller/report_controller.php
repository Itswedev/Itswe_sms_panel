...
if(isset($_REQUEST['list_type']))
{
	...
	else if($list_type=='reschedule_campaign')
    {

    		

			echo $reschedule_rs=reschedule_campaign();
			...
			array('db' => 'job_id', 'dt' => 0,
				'formatter' => function($d, $row) {
					...
					session_start();
						...
						return "
							<form method='POST' action='dashboard.php?page=send_job' style='display:inline;' id='form_job_$d'>
								<input type='hidden' name='job_id' value='$d'>
								<input type='hidden' name='table_name' value='$table'>
								<input type='hidden' name='job_date' value='{$row[1]}'>
								<a href='#' onclick='document.getElementById(\"form_job_$d\").submit(); return false;'>$d</a>
							</form>";
		   	...
		   	array('db' => 'msg_credit','dt' => 7,'formatter' => function($d, $row) {
	        	...
	        	array('db' => 'job_id','dt' => 9,'formatter' => function($d, $row) {
					...
					{
						global $dbc;
						$btn1='<button type="button" class="btn btn-primary view_delivery_count" alt="Delievry count" data-item-id="'.$d.'"><i class="fa fa-eye"></i></button><br><b><span id=delivery_count_'.$d.'></b></span><br/><button type="button" class="btn btn-primary repush_btn" alt="Repush Job" data-item-id="'.$d.'" data-bs-toggle="modal" data-bs-target="#repush_job_modal"><i class="fa fa-upload"></i></button><br/>';

						$query_picked="select is_picked from $table where job_id='".$d."'";
						$is_picked_res=mysqli_fetch_array(mysqli_query($dbc,$query_picked));

						 $is_picked=$is_picked_res['is_picked'];
						
						if($is_picked==0)
						{
							$btn2='<button type="button" class="btn btn-primary pause_btn" alt="Pause" data-item-id="'.$d.'"><i class="fa fa-pause"></i></button><br><b><span id=pause_'.$d.'></b></span>';	
							return $btn1."<br/>".$btn2;
						}
						else if($is_picked==2)
						{
							$btn2='<button type="button" class="btn btn-primary play_btn" alt="Play" data-item-id="'.$d.'"><i class="fa fa-play"></i></button><br><b><span id=play_'.$d.'></b></span>';	
							return $btn1."<br/>".$btn2;
						}
						else{
							return $btn1;
						...
						$btn1='<button type="button" class="btn btn-primary view_delivery_count" alt="Delievry count" data-item-id="'.$d.'"><i class="fa fa-eye"></i></button><br><b><span id=delivery_count_'.$d.'></b></span>';
		...
		if($row_count>0)
		{
			...
			if ($result_zip === TRUE) 
			{
					    while($row=mysqli_fetch_array($result))
							...
							//  $record.="<td>".$total_records."</td><td><a href='controller/".$fileName."' download><button type='button' class='btn btn-primary' id='download_report_btn'>Download
							...
							<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-download' viewBox='0 0 16 16'>
								<path d='M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z'></path>
								<path d='M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 1 0-1 0v8.793l-3.146-3.147a.5.5 0 0 0-.708.708l3 3z'></path>
							</svg>
							</button></a></td>";
			   		...
			   		//   $record_query="SELECT DATE_FORMAT(sent_at, '%Y-%m-%d %H:%i:%s') AS 'Job Date', userids as 'Userid', CONVERT(SUBSTRING_INDEX(metadata, 'TID=', -1),CHAR) as 'Template ID', msg_job_id as 'Job ID', char_count as 'Chars', route as 'Route', senderid as 'Sender ID', msgcredit as 'Bill Credit', mobile_number as 'Mobile Number', status as 'Status', err_code as 'Error code', msgdata as 'Message', DATE_FORMAT(FROM_UNIXTIME(delivered_date), '%Y-%m-%d %H:%i:%s') AS 'DLR Time' from $sendtabledetals where $extraWhere limit $start,400000 ";
//  $record_query = "SELECT DATE_FORMAT(sent_at, '%Y-%m-%d %H:%i:%s') AS 'Job Date',userids AS 'Userid', 
//   CONCAT('\'\\\\\'', SUBSTRING_INDEX(metadata, 'TID=', -1), '\'\\\\\'') AS 'Template ID',
...
//                        msgdata AS 'Message',
//                        DATE_FORMAT(FROM_UNIXTIME(delivered_date), '%Y-%m-%d %H:%i:%s') AS 'DLR Time'
//                FROM  $sendtabledetals
//                WHERE $extraWhere
//                LIMIT $start, 900000";	
...
error_reporting(E_ALL);*/
	...
	else if($list_type=='api_job_table_dtls_uat')
    {
    			global $dbc;
			...
			$columns = array(
			array( 'db' => 'id','dt' => 0 ),
			array( 'db' => 'userids','dt' => 1 ,'formatter' => function( $d, $row ) {
				$username=get_username($d);
				  return $username;
			}),
			    array( 'db' => 'route','dt' => 2 ),
			    array( 'db' => 'mobile_number','dt' => 3 ,'formatter' => function( $d, $row ) {
			    ...
			    array( 'db' => 'msgdata','dt' => 4 ,'formatter' => function( $d, $row ) {

			    			$msg=urldecode($d);
			    				return "<div style='word-break: break-all;'>$msg</div>";
			           
			        }),
			    array( 'db' => 'char_count','dt' => 5 ),
			    array( 'db' => 'msgcredit','dt' => 6),
			    array( 'db' => 'master_job_id','dt' => 7),
			   /* array( 'db' => 'id','dt' => 7),*/
			    array( 'db' => 'status','dt' => 8),
			    array( 'db' => 'sent_at','dt' => 9),
			    array( 'db' => 'delivered_date','dt' => 10,'formatter' => function( $d, $row ) {
			...
			if($user_role=='mds_adm')
			{
				if($extraWhere!="")
			    {
					$extraWhere.=" and is_scheduled=0 and schedule_sent=1 and circle='API' ";
				}
				else
				{
					$extraWhere.=" is_scheduled=0 and schedule_sent=1  and circle='API'";
				}
			}
			else
			{
				if($extraWhere!="")
			    {
					$extraWhere.=" and is_scheduled=0 and schedule_sent=1 and circle='API' and userids='".$userid."' ";
				}
				else
				{
					$extraWhere.=" is_scheduled=0 and schedule_sent=1  and circle='API' and userids='".$userid."'";
				}
			}
     ...
     else if($list_type=='rcs_job_table_dtls')
    {
		   	...
		   	$today_dt=date("Y-m-d");
			$curr_month=date('m');
			$job_m=date("m",strtotime($_REQUEST['job_date']));
		   	$yesterday_dt=date("Y-m-d",strtotime("-1 day"));
		   	if($job_m!=$curr_month)
		   	{
		   		$report_yr=date("Y",strtotime($job_date));
 		...
 		else if($list_type=='schedule_report')
    {
    	...
    	$sendtabledetals = SENDSMSDETAILS;
			    ...
			    array(
			      ...
			      array(
			       ...
			       array( 'db' => 'msg_credit','dt' =>6,'formatter' => function( $d, $row ) {
					...
					array( 'db' => 'numbers_count','dt' => 7),
					array( 'db' => 'senderid','dt' =>8,'formatter' => function( $d, $row ) {
			            return get_senderid_name($d);
			        }),
					array( 'db' => 'schedule_sent','dt' =>9,'formatter' => function( $d, $row ) {
					...
					array( 'db' => 'request_code','dt' =>10,'formatter' => function( $d, $row ) {
			            return '-';
			        }),
			        array( 'db' => 'id','dt' =>11,'formatter' => function( $d, $row ) {
			        	global $table;
						$button="<button class='btn btn-primary me-1 mb-1 delete_message_job_btn' type='button' data-id='".$d."' data-tblname='".$table."'>
						<i class='fa fa-trash'></i>
						  ...
						  else
						  {
							...
							$job_id=$row[2];
							$old_date=$row[5];
							$button_reschedule="<button class='btn btn-primary me-1 mb-1 reschedule_message_job_btn' type='button' data-id='".$d."' data-job_id='".$job_id."' data-tblname='".$table."' data-old_date='".$old_date."'>
						  <i class='fa fa-pencil'></i>
							</button>";
							  $button.=$button_reschedule;
						  }
			...
			else
			{
					...
					$toDate=$today_dt;
					$currentMonth = date('m');
			    $extraWhere.="(month(sent_at) =$currentMonth) and is_scheduled=1";
			}
			...
			if($user_role=='mds_adm')
				{		
						...
						if($selected_role=="User")
						{
							...
							{
								$check_user_ids=$select_user_id;
								$extraWhere.=" and userid in (".$check_user_ids.")  and is_scheduled=1";
							}
							else
							{
								$extraWhere.=" and is_scheduled=1";
							}
						}
					else if($selected_role=="Reseller")
						{
				...
				else if($user_role=='mds_sub_usr')
				{
						...
						if($u_id=='All')
						{
								...
								$userid = $user_tree;
						 		if($extraWhere!="")
											{
												$extraWhere.=" and `userid` in ($userid)  and is_scheduled=1 ";
											}
											else
											{
												$extraWhere="`userid` in ($userid)  and is_scheduled=1 ";
											}
						}
						else
						{
				...
				else if($user_role=='mds_rs' || $user_role=='mds_ad')
				{
                   ...
                   if(!empty($check_user_ids))
                   {
	                   		if($extraWhere!="")
											...
											{
												$extraWhere="`userid` in ($check_user_ids)  and is_scheduled=1 ";
											}
                   }
							}
							else
                   	...
                   	if($extraWhere!="")
											{
												$extraWhere.=" and `userid`=$userid  and is_scheduled=1 ";
											}
											else
											{
												$extraWhere="`userid`=$userid  and is_scheduled=1 ";
											}
							}
						}
						else if($selected_role=="Reseller")
...
function reschedule_campaign()
{
	global $dbc;
	$u_id=$_SESSION['user_id'];

	$job_id = $_REQUEST['msg_id'];
	$old_date = $_REQUEST['old_date'];
	$schedule_date = $_REQUEST['schedule_date'];
	$senddtlstable=SENDSMSDETAILS;
	$sendtable = SENDSMS . CURRENTMONTH;
	$today_dt=date("Y-m-d h:i:s");
	$dateTime = DateTime::createFromFormat('Y-m-d h:i A', $schedule_date);
	$formattedDate = $dateTime->format('Y-m-d H:i:s');
	
	
	if($formattedDate<=$today_dt)
	{
		return "Back date selection for schedule not allowed!!";
	}
	else
	{
		...
		if(!empty($job_id))
		{
			$update_camp="update $senddtlstable set sent_at='".$formattedDate."' where msg_job_id='".$job_id."'";
			$result=mysqli_query($dbc,$update_camp);

			if($result)
			{
				$update_job="update $sendtable set sent_at='".$formattedDate."' where job_id='".$job_id."'";
				$result_job=mysqli_query($dbc,$update_job);
				if($result_job)
				{
					return "Campaign successfully rescheduled!!!";
				}
				else{
					return "Failed to reschedule campaign";
				}
