   ...
   $.ajax({
    ...
    $("#dynamic_sms_btn").click(function(){
        $( "#sendSMSForm" ).validate({
            ...
            e.preventDefault();
            //console.log('Form submitted');
            var preview=$("#txtpreview_file").val();
            if(preview=="")
            {
                  swal.fire({
                              icon: 'error',
...
$("#btn_default_val").click(function(){
    ...
    $("#preview").click(function(){
     ...
     /* $('input[name^=default_value]').map(function(idx, elem) {
        default_value.push($(elem).val());
    
  }).get();
*/

/*      $("input[name=default_value]").each(function() {
    ...
    //var full_url = window.location.origin;
            $.ajax({
                    ...
                    type: 'post',
                    cache: false, 
                    data:"act=preview2&file_name="+file_name+"&msg="+msg,
                    success: function(data){
                        var d = data.split('|||');
                        $("#preview_area").html('');
