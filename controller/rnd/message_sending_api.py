...
def send_master_tbl(message_id,job_id):
        ...
        master_tbl_qry = "SELECT id,mobile_number,senderid,msgcredit,unicode_type,userids,service_id,is_scheduled,request_code,send_msg,metadata,route FROM " + sendtabledetals + " WHERE is_picked=0 AND (is_scheduled=0) and message_id='"+str(message_id)+"' and msg_job_id='"+str(job_id)+"' and cut_off='No' and `status`='Submitted' ORDER BY id ASC LIMIT 5000"
...
def update_job_tbl(message_id,job_id):
    ...
    mycursor1.execute(select_master_tbl_count_qry)
    myresult_master_count_tbl = mycursor1.fetchall()
    ...
    mycursor1.execute(select_job_tbl_count_qry)
    myresult_job_count_tbl = mycursor1.fetchall()
    ...
    if total_master_record == total_job_record :
        ...
        mycursor1.execute(select_master_tbl_qry)
        myresult_master_tbl = mycursor1.fetchall()
        ...
        for master_row in myresult_master_tbl:
            ...
            if pending_cnt == 0 :
                ...
                # print(update_master_tbl_qry)
                mycursor1.execute(update_job_tbl_qry)
                mydb1.commit()
    else:
    ...
    # mycursor1, mydb1 = connection.get_connection()
        ...
        while True:
            try:
                ...
                yr = today.strftime("%Y")
                mo = today.strftime("%m")
                send_job_tbl= "sendmessages_api"+yr+mo
                ...
                if(send_sms_count < 50000):
                    job_tbl_qry = "SELECT id,job_id FROM " + send_job_tbl + " WHERE is_picked=0 AND (is_scheduled=0 )  ORDER BY id DESC LIMIT 100"
                    mycursor1.execute(job_tbl_qry)
                    myresult_send_job = mycursor1.fetchall()
