...
remove_rcs_master_table_record();
    function remove_rcs_master_table_record()
    {
        ...
        $sql="select * from $sendtabledetals where date(sent_at)='$record_dt'";
       ...
       if($count>0)
       {
        ...
        $monthly_archive_table=RCSMASTER.$record_year.$record_month;
        

        $sql_insert="insert into $monthly_archive_table select * FROM $sendtabledetals WHERE date(sent_at)='$record_dt'";
        ...
        /* echo "select count(1) as count_archive from $monthly_archive_table where date(created_at)='$record_dt'";*/
         $archive_count_res=mysqli_query($dbc,"select count(1) as count_archive from $monthly_archive_table where date(sent_at)='$record_dt' ");
         ...
         if($count_archive==$count)
         {
           /* echo "Successfully Inserted into archive table. Record Success Date:- ".$record_dt;*/
            ...
            $sql_del="delete FROM $sendtabledetals WHERE date(sent_at)='$record_dt'";
    ...
    //    $myfile = fopen("/var/www/html/itswe_panel/controller/logfiles/remove_rcs_master_table_record_log.txt", "a") or die("Unable to open file!");
