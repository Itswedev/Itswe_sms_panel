...
if(isset($schedule_time_arr))
{
	for($i=0;$i<count($schedule_time_arr);$i++) {
		...
		if($schedule_time==$today_dt)
		{
			...
			exec("php /var/www/html/itswe_panel/controller/classes/schedule_sms/$executable_php $executable_json $msg_id > /dev/null 2>/dev/null & > /var/www/html/itswe_panel/controller/classes/schedule_sms/log.txt"); 
		}
		else
		{
			echo 0;
