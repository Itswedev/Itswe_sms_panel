...
function upload_csv_file()
{
	...
	new AjaxUpload(btnUpload, {
		...
		onSubmit: function(file, ext) {
			...
			if (! (ext && /^(txt|xls|csv|xlsx|CSV)$/.test(ext))) {
...
function checkChar() 
{
	var char_set=$("input[name='char_set']:checked").val();
	//console.log(char_set);
	var msg2 = $.trim($('#mbl1').val());
	...
	if(charcount > SMSMAX)
	{
		...
		$("#msg_credit").val(Credit);
		$("#msg_chars").val(charcount);
