...
$("#send_sms").click(function(){
    ...
    if($('#chk_track').is(":checked")){ 
        ...
        $( "#sendSMSForm" ).validate( {
            ...
            var mob_count=$("#counti").text();
            var sid = $('#sid option:selected').text();
            
            var total_credits=smscount*mob_count;

            swal.fire({
            title: 'Are you sure?',
            html: "You want to send this message!<br><br><table border='1' ><tr><th> Message : </th> <td>"+msg+"</td></tr><tr><th>Total Credits : </th> <td>"+total_credits+"</td></tr><tr><th>Senderid : </th> <td>"+sid+"</td></tr>",
                       ...
                       // window.location.reload();
                            if(msg[1]!='0'||msg[1]!=0)
                            {
                                ...
                                swal.fire('',msg[0]+"<br/>"+msg[1],'success').then((value) => {
                                  ...
                                  window.location.href=full_url+"/dashboard.php?page=archive_report";
...
$("#schedule_now_btn").click(function(){
    ...
    if($('#chk_track').is(":checked")){ 
        ...
        $( "#sendSMSForm" ).validate( {
            ...
            swal.fire({
            title: 'Are you sure?',
            html: "You want to send this message!<br><br><center><table border='1' style='margin-left: 22%;' ><tr><th> Message : </th> <td>"+msg+"</td></tr><tr><th>Total Credits : </th> <td>"+total_credits+"</td></tr></table></center>",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#2c7be5',
            cancelButtonColor: '#748194',
...
$("#schedule_sms").click(function(){
    ...
    if($('#chk_track').is(":checked")){ 
        ...
        $( "#sendSMSForm" ).validate( {
            ...
            swal.fire({
  title: 'Are you sure?',
  html: "You want to schedule this message!<br><b> Message </b>: "+msg+"<br/><b>Total Credits</b>: "+total_credits,
  icon: 'warning',
  showCancelButton: true,
  confirmButtonColor: '#2c7be5',
  cancelButtonColor: '#748194',
...
$("#save_template_btn").click(function(){
    var selectedRadio = $('input[class="select_temp_id"]:checked');
    
    if (selectedRadio.length > 0) {
        // Retrieve the data attributes of the selected radio button
        var tempId = selectedRadio.data('id');
        var tempName = selectedRadio.data('temp_name');
        var msg = selectedRadio.data('msg');
        var template_id = selectedRadio.data('template_id');
        var charType = selectedRadio.data('char_type');
        var temp_option="<option value="+tempId+">"+tempName+"</option>";
        $('#template').append(temp_option);
        $("#template").val(tempId);
        $("#dlt_template").val(template_id);
        $("#mbl1").val(msg);
        checkChar();
        // Output the values or perform any action
        // console.log('Selected Template ID:', tempId);
        // console.log('Selected Template Name:', tempName);
        // console.log('Message:', msg);
        // console.log('Character Type:', charType);
        $("#select_template_modal").modal('hide');
        // Example: Display the selected values in an alert
        //alert(`Selected Template ID: ${tempId}\nTemplate Name: ${tempName}\nMessage: ${msg}\nCharacter Type: ${charType}`);
    } else {
