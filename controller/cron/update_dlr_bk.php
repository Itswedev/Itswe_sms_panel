...
update_master_table();
    function update_master_table()
    {
     ...
     echo $QuerySelect= "SELECT `smsc_id`,`time`,`msgdata`,`dlr_url` FROM $sent_sms WHERE update_dlr=0 and `momt`='DLR'  limit 10000";
     echo "<br/>";
                      ...
                      while ($row = mysqli_fetch_array($result)) {
                         ...
                         $data['time']   =  $row[1];
                          ...
                          else
                          {
                                ...
                                $stat=getDlrStatusDetails($err);
                          }


                         echo  $queryUpdate="Update $sendtabledetals set service_id='$smsc_id',err_code='$err',status='$stat' where id=$dlr_id";        
                         echo "<br>";                   
    ...
    function getDlrStatusDetails($err_code) {
        global $dbc;
        if ($err_code != "") {
            $q = "SELECT `err_status` FROM tbl_errorcode WHERE  err_code = '{$err_code}'";
