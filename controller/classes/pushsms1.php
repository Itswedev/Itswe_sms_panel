...
error_reporting(0);
class pushsms1_uat extends common{

    /* --- sendQuickSMSSave --- */
    function sendQuickCALLSave($userid) {
        ...
        $retry_attempt=$_REQUEST['retry_attempt'];
        $retry_duration=$_REQUEST['retry_duration'];
        $call_method=$_REQUEST['method'];
        $wait_duration=$_REQUEST['wait_duration'];
        $form_type='Multimedia';
        $credit_refund=0;
        ...
        $udh=15;
         ...
         $inhouse_userids = array("4645", "4752", "6028", "6027","6101");
        if (in_array($u_id, $inhouse_userids))
        {
            $udh=30;
        }
        ...
        $voice_file = $service_name = "";
        if (isset($_POST['voice_file'])) 
        {
            $voice_file = $_POST['voice_file'];
            $voice_file = trim($voice_file);
            $multimedia_id=$voice_file;
           /* $route_time= $this->fetch_route_time($voice_file);
             if($route_time!=1)
                {
                     return array('status' => false, 'msg' => 'Unreliable message sending time');
                    exit;
                }*/
            $file_name= $this->fetch_file_name($voice_file);
            //$dnd_status= $this->fetch_dnd_status($voice_file);

            $planid=$this->fetch_plan(7);
            $gateway_id =$this->fetch_gateway_id($planid,7);
            $service_name =$this->fetch_gateway_name($planid,7);

            $gateway_name=$service_name['gateway_name'];

            $end_point=$service_name['end_point'];
        }
        ...
        if($gateway_name=='')
        {
            return array('status' => false, 'msg' => 'Incorrect Vendor selection','plan'=>$planid,'service_name'=>$gateway_name);
            exit;
        }
       
        if ($_POST['voice_file'] == '' || $_POST['numbers'] == '' ) {
            return array('status' => false, 'msg' => 'EmptyField');
            exit;
        }

         $sql_fetch_multimedia="select get_response,file_duration,voice_id,sender_id,template_id,ivr_id,ivr_id2 from multimedia where id='".$multimedia_id."'";
        $multimedia_result=mysqli_fetch_array(mysqli_query($dbc, $sql_fetch_multimedia));
        $get_response=$multimedia_result['get_response'];
        $file_duration=$multimedia_result['file_duration'];
        $voice_id=$multimedia_result['voice_id'];
        $ivr_id=$multimedia_result['ivr_id'];
        $ivr_id2=$multimedia_result['ivr_id2'];
        $sender_id=$multimedia_result['sender_id'];
        $template_id=$multimedia_result['template_id'];
        $total_credit=$file_duration+$wait_duration;
        $msg_len=$total_credit;
         if ($msg_len > $SMS) {
        ...
        $userid = $_SESSION['user_id'];
            $idss = $this->getOverSeelingUserids($_SESSION['user_id']);
                    ...
                    $ids = implode(',', $idss);
                    $userids = $ids;
                    $out = $this->checkBalance($idss, $az_routeid=7, $credit);
                    if($out!='')
                    {
                            if ($out == false) {
                            ...
                            $usrcredit = $this->userCreditBalance($userid, $az_routeid=7);
        ...
        if (isset($_POST['is_schedule']) && $_POST['is_schedule'] != '') {
            ...
            $req = "#1";
            $sent_at=$schdate;
            $sendtable = SENDCALL . $this->getScheduleDateTimeMonth($_POST['scheduleDateTime']);
       ...
       // $campaign_type = isset($_POST['campaign_name']) && !empty($_POST['campaign_name']) ? $_POST['campaign_name'] : "";
        ...
        {
            $cutstatus="Yes";
             $q = 'INSERT INTO ' . $sendtable . ' (`id`, `message`, `userid`, `campaign_id`, `is_scheduled`, `scheduled_time`, `created_at`, `updated_at`, `unicode_type`, `senderid`, `service_id`, `sms_type`, `is_refund`, `other_operator`, `request_code`, `campaign_name`, `is_picked`, `numbers_count`, `msg_credit`,  `charset`, `senderid_name`,  `original_url`, `url_status`, `pe_id`, `template_id`,`gvsms`,`ip_address`,`method`,`form_type`,`cut_off_value`,`route`,`job_id`,`schedule_sent`,`sent_at`,`cut_off`,`cut_off_throughput`,`total_cutting`,`caller_id`,`call_method`,`wait_duration`,`auto_response`) VALUES (NULL, "' . $msg . '", "' . $userid. '", "' . $campaign_id . '", "' . $is_schedule . '", "' . $schdate . '", NOW(), NOW(), "'.$text_type.'", "' . $sid . '" , "' . $az_routeid . '", "' . $_POST['send_type'] . '", ' . $is_refund . ', NULL, "", "' . $campaign_name . '", 1, "' . $count . '", "' . $msgcredit . '",  "utf-8", "' . $senderid_name . '",  "' . $_POST['original_url'] . '", ' . $url_status . ', "' . $pe_id . '", "' . $template_id . '", "' . $gvsms . '", "' . $ip_address . '"
            , "' . $method . '", "' . $form_type . '", "0","'.$route_name.'","'.$job_id.'","'.$schedule_sent.'","'.$sent_at.'","'.$cutstatus.'","'.$random_cutoff.'","'.$count_cutoff_data.'","'.$caller_id.'","'.$call_method.'","'.$wait_duration.'","'.$get_response.'")';

             $credit_refund=$count_cutoff_data*$msgcredit;
        }
        else
        {
             $q = 'INSERT INTO ' . $sendtable . ' (`id`, `job_id`, `msg_format`, `is_scheduled`, `scheduled_time`, `created_dt`, `campaign_name`, `numbers_count`, `msgcredit`,`vsms`,`ip_address`,`method`,`cut_off_value`,`route`,`schedule_sent`,`sent_at`,`userid`,file_name,`multimedia_id`,`retry_attempt`,`retry_duration`,`caller_id`,`call_method`,`wait_duration`,`auto_response`,`service_id`) VALUES (NULL, "' . $job_id . '", "' . $method. '", "' . $is_schedule . '", "' . $schdate . '", NOW(), "' . $campaign_name . '", "' . $count . '", "' . $msgcredit . '",  "'.$gvsms.'", "' . $ip_address . '"
            , "' . $method . '","0","'.$route_name.'", "'.$schedule_sent.'","'.$sent_at.'","'.$userid.'","'.$file_name.'","'.$multimedia_id.'","'.$retry_attempt.'","'.$retry_duration.'","'.$caller_id.'","'.$call_method.'","'.$wait_duration.'","'.$get_response.'","'.$gateway_name.'")';
        }
       ...
       /* return array('status' => false, 'rs' => $rId,'job_id' => $job_id,'qry' => $q);
            exit;*/
       // return array('status' => false, 'rid' => $rId);
        ...
        if ($url_status == 1) {
            ...
            foreach ($v_num as $value) 
            {
                    ...
                    //$service_name='LIVE';
                     if(!empty($dnd_num))
                        ...
                        $trans_id=$this->random_strings(18);
                        $cut_off_status="No";
                        $str[]= '(NULL, "' . $rId . '", "'.$num.'", NOW(), "' . $is_schedule . '", "' . $schdate . '", "' . $msgcredit . '", "' . $userids . '","' . $status . '", "' . $err_code . '",  "'.$route_name.'","'.$job_id.'", "'.$schedule_sent.'", "'.$parent_id.'", "'.$sent_at.'","'.$cut_off_status.'","'.$master_job_id.'","'.$multimedia_id.'","'.$file_name.'","'.$caller_id.'","'.$trans_id.'","'.$retry_duration.'","'.$gateway_name.'")';
            ...
            $call_data = array_chunk($call_send, 5000);
            if($_POST['is_schedule'] != '1')
            {
            
            get_last_activities($u_id,"Multimedia SMS Send. Job Id: $job_id",@$login_date,@$logout_date);
        
            $is_schedule=$_REQUEST['is_schedule'];

            // $array = array('msg_job_id'=>$job_id,'call_send'=>$call_send,'multimedia_id'=>$multimedia_id,'retry_attempt'=>$retry_attempt,'retry_duration'=>$retry_duration,'caller_id'=>$caller_id,'vsms'=>$gvsms,'userid'=>$userids,'parent_id'=>$parent_id,'file_name'=>$file_name,'msg_id'=>$rId,'call_method'=>$call_method,'wait_duration'=>$wait_duration,'msgcredit'=>$msgcredit,'gateway_name'=>$gateway_name,'end_point'=>$end_point,'campaign_name'=>$campaign_name);
            $array = array('msg_job_id'=>$job_id,'call_send'=>$call_send,'query_data'=>$query_data,'multimedia_id'=>$multimedia_id,'retry_attempt'=>$retry_attempt,'retry_duration'=>$retry_duration,'caller_id'=>$caller_id,'vsms'=>$gvsms,'userid'=>$userids,'parent_id'=>$parent_id,'file_name'=>$file_name,'msg_id'=>$rId,'call_method'=>$call_method,'wait_duration'=>$wait_duration,'msgcredit'=>$msgcredit,'gateway_name'=>$gateway_name,'end_point'=>$end_point,'voice_id'=>$voice_id,'sender_id'=>$sender_id,'template_id'=>$template_id,'campaign_name'=>$campaign_name,'ivr_id'=>$ivr_id,'ivr_id2'=>$ivr_id2);
            $filename="send_call_data".time().".json";
            $file_path="classes/sent_call/".$filename;
            $fp = fopen($file_path, 'w+');
            fwrite($fp, json_encode($array, JSON_PRETTY_PRINT));   // here it will print the array pretty
            fclose($fp);

            $new_send_file="sent_call_".time().".php";
            copy("/var/www/html/controller/classes/sent_call.php", "/var/www/html/controller/classes/sent_call/".$new_send_file);
    
            exec("php /var/www/html/controller/classes/sent_call/$new_send_file $filename > /dev/null 2>/dev/null &"); 

            }
            else
            {
                   // $sendtabledetals=SENDCALLSCHEDULE;
                    ...
                    /*    $query_master_tbl_insert= "INSERT INTO $sendtabledetals (`id`, `created_dt`, `msgcredit`, `userid`, `status`, `route`,`msg_job_id`,`schedule_sent`,`parent_id`,`sent_at`,`cut_off`,`master_job_id`,`multimedia_id`,`file_name`,`caller_id`,`trans_id`,`message_id`,`vsms`,`retry_duration`,`mobile_number`) VALUES $insert_val";*/
                        ...
                        /*  $msg_arr[]=$values;*/
                        $query_master_tbl_insert= "INSERT INTO $sendtabledetals (`id`, `message_id`,  `mobile_number`, `created_dt`,  `is_scheduled`, `scheduled_time`, `msgcredit`, `userid`,  `status`, `err_code`,`route`,`msg_job_id`,`schedule_sent`,`parent_id`,`sent_at`,`cut_off`,`master_job_id`,`multimedia_id`,`file_name`,`caller_id`,`trans_id`,`retry_duration`,`service_id`) VALUES $insert_val";
                    ...
                    $new_send_file="run_voice_schedule_sms".$job_id.".php";
                    copy("/var/www/html/controller/classes/run_voice_schedule_sms.php", "/var/www/html/controller/classes/schedule_voice/".$new_send_file);

                    $array_schedule = array('php_file' => $new_send_file,'schedule_time' =>$schdate,'message_id' =>$job_id,'gateway_id' =>$gateway_id);
                    $schedule_filename="voice_scheduler.json";
            ...
            if(!empty($user_ids))
            {
                ...
                $url = "https://mdssend.in/api.php?username=sam&apikey=m3Sr9ufrPDaj&senderid=MDSACC&route=TRANS&mobile=$mobile_nos&text=$msg";
    ...
    /* /. --- sendQuickSMSSave --- ./ */
      ...
      /* --- schedule later --- */
      function sendScheduleCALLSave($userid) {
        ...
        foreach ($scheduleData as $schedule_date => $numbers) 
        {
            ...
            $req = "#1";
            $sent_at=$schdate;
            $sendtable = SENDCALL . $this->getScheduleDateTimeMonth($schedule_date);
            $sendtabledetals = SENDCALLDETAILS ;
            } else {
                $is_schedule = 0;
                $schedule_sent=1;
    ...
    public function divideMobileNumbersEqually($mobileNumbers, $dates) {
...
function fetch_sender_name($sid)
{
    ...
    function getSenderIdName($sid = null) {
        ...
        if (!empty($routeid)) {
            ...
            if(!empty($gatewayid))
            {
                 $q = "SELECT `end_point`,`gateway_name` FROM voice_gateway WHERE gateway_id='$gatewayid'";
                $rs = mysqli_query($dbc, $q);
                $row = mysqli_fetch_assoc($rs);
    ...
    function fetch_gateway_id($planid=null,$routeid = null) {
        global $dbc;
        if (!empty($routeid)) {
            $out = array();
            $q = "SELECT `gateway_id` FROM az_route_plan WHERE  `plan_id`='$planid' and `route_id`='$routeid' and rp_status = 1";
     ...
     function fetch_file_name($multimedia_id = null) {
        global $dbc;
        if (!empty($multimedia_id)) {
    
            $q = "SELECT `original_filename` FROM multimedia WHERE `id`='".$multimedia_id."' and status = 1";
            $rs = mysqli_query($dbc, $q);
           while($row = mysqli_fetch_assoc($rs))
           {
     ...
     function fetch_dnd_status($multimedia_id = null) {
        global $dbc;
        if (!empty($routeid)) {
            $out = array();
            $q = "SELECT `dnd_enable` FROM multimedia WHERE  `id`='$multimedia_id' and status = 1";
            $rs = mysqli_query($dbc, $q);
            $row = mysqli_fetch_assoc($rs);
