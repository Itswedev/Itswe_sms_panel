...
$(function() {
	...
	/*if(page_name=="Live Jobs"){
		...
		$(document).on('click','#download_report_btn',function(){
			...
			/*$("#send_job_details_tbl").table2excel({

			    // exclude CSS class

			    exclude:".noExl",

			    name:"Send Job Report",

			    filename:"Send Job Report",//do not include extension
...
function load_live_job_report()
{
		...
		table= $('#live_job_report_tbl').DataTable({
        	"processing": true,
        	"serverSide": true,
...
function load_live_job_data()
{
	...
	/*alert(job_date);*/
        ...
        type: 'post',
        cache: false, 
        data:'list_type=live_job_data&job_id='+job_id+"&table_name="+table_name+"&job_date="+job_date,
        dataType:'json',
        success: function(data){
