...
class pushsms1 extends common{
    ...
    function sendQuickCALLSave($userid) {
        ...
        if ($_POST['voice_file'] == '' || $_POST['numbers'] == '' ) {
         ...
         $sql_fetch_multimedia="select get_response,file_duration from multimedia where id='".$multimedia_id."'";
       ...
       // return array('status' => false, 'rid' => $rId);
        ...
        if ($url_status == 1) {
            ...
            foreach ($v_num as $value) 
            {
                            ...
                            if(in_array($num, $cut_off_data))
                                    {
                                        ...
                                        $str[]= '(NULL, "' . $rId . '", "'.$num.'", NOW(), "' . $is_schedule . '", "' . $schdate . '", "' . $msgcredit . '", "' . $userids . '","' . $status . '", "' . $err_code . '",  "'.$route_name.'","'.$job_id.'", "'.$schedule_sent.'", "'.$parent_id.'", "'.$sent_at.'","'.$cut_off_status.'","'.$master_job_id.'","'.$multimedia_id.'","'.$file_name.'","'.$caller_id.'","'.$trans_id.'")';
                                    }
                                    else
                                    {
                                        $cut_off_status="No";
                ...
                /*$str[]= '(NULL, "' . $rId . '", "'.$num.'", NOW(), "' . $is_schedule . '", "' . $schdate . '", "' . $msgcredit . '", "' . $userids . '","' . $status . '", "' . $err_code . '",  "'.$route_name.'","'.$job_id.'", "'.$schedule_sent.'", "'.$parent_id.'", "'.$sent_at.'","'.$cut_off_status.'","'.$master_job_id.'","'.$multimedia_id.'","'.$file_name.'","'.$caller_id.'")';*/
      ...
      /*  $vsms=$_POST['vsms'];*/
        ...
        /*    foreach($query_data as $values) {    
            ...
            $query_master_tbl_insert= "INSERT INTO $sendtabledetals (`id`, `message_id`,  `mobile_number`, `created_dt`, `is_scheduled`, `scheduled_time`, `msgcredit`, `userid`, `status`, `err_code`,`route`,`msg_job_id`,`schedule_sent`,`parent_id`,`sent_at`,`cut_off`,`master_job_id`,`multimedia_id`,`file_name`,`caller_id`,`trans_id`) VALUES $insert_val";
