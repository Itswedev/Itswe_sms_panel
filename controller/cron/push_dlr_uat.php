...
while(true)
{
    ...
    function push_dlr()
    {
        global $dbc;
        //$sent_sms='sent_sms';
        $sendtabledetals = "az_sendnumbers202407";
        echo "\n Log started......\n";
        echo "\n started at".date("Y-m-d h:i:s");
        echo "\n";
        
        $QuerySelect= "SELECT id,msg_job_id,mobile_number,`status`,unicode_type,err_code,delivered_date,send_msg,msgdata FROM $sendtabledetals WHERE status_id=0 and `status`!='Submitted' and date(sent_at)='2024-07-21' and userids=29 and schedule_sent=1 and is_picked=1 limit 200";
                ...
                $result = mysqli_query($dbc, $QuerySelect);
                 ...
                 if (mysqli_num_rows($result) > 0) {
                      ...
                      $ids=implode(",",$id);
                      $sql_update="update az_sendnumbers202407 set status_id=1 where id in ($ids)";
