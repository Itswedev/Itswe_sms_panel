...
def update_otp_thread_function():
    ...
    while True:
        try:
            ...
            get_err_code = '045'
            today = date.today()
            QuerySelect = "SELECT `smsc_id`,`time`,`dlr_url`,LOCATE('NACK',msgdata),`dlr_mask`,SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(url_decode2(msgdata),' ', -2),' ',1),'err:',-1) FROM "+sent_sms+" WHERE update_dlr=0 and `momt`='DLR'  limit 50000"
