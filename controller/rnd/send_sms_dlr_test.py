...
def update_otp_thread_function():
    ...
    while True:
        try:
            ...
            get_err_code = '045'
            today = date.today()
            QuerySelect = "SELECT `smsc_id`,`time`,`dlr_url`,LOCATE('NACK',msgdata),`dlr_mask`,SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(url_decode2(msgdata),' ', -2),' ',1),'err:',-1) FROM "+sent_sms+" WHERE `momt`='DLR' and dlr_url='102361109' order by sql_id desc "
            # print(QuerySelect)
            ...
            if dlr_count > 0:
                    ...
                    if msgdata_nack > 0 :
                       ...
                       master_ids.append(dlr_url)
                        ...
                        if dlr_mask == 1:
                            ...
                            if status_count > 0 :
                                status_query ="select err_status from tbl_errorcode where err_code='"+str(get_err_code)+"' and gateway_id='"+str(gateway_id)+"' limit 1"
                                # print(status_query)
                                # print(dlr_url)
                ...
                mycursor4.execute(update_sent_sms_tbl_qry)
                mydb4.commit()
