...
remove_smartcutoff_table_record();
    function remove_smartcutoff_table_record()
    {

        global $dbc;
        $response="Cron start time:- ".date('Y-m-d h:i');
        $smartcutoffdetals = SMARTCUTOFF;
        //$smartcutoffdetals = 'az_sendnumbers_bk';
       ...
       $sql="select * from $smartcutoffdetals where date(created_date)='$record_dt'";
       ...
       if($count>0)
       {
        ...
        $monthly_archive_table='smart_cutoff'.$record_year.$record_month;
        

        $sql_insert="insert into $monthly_archive_table select * FROM $smartcutoffdetals WHERE date(created_date)='$record_dt'";

         /*echo "<br>";*/
         $result_insert=mysqli_query($dbc,$sql_insert) or die(mysqli_error($dbc));
        /* echo "select count(1) as count_archive from $monthly_archive_table where date(created_date)='$record_dt'";*/
         $archive_count_res=mysqli_query($dbc,"select count(1) as count_archive from $monthly_archive_table where date(created_date)='$record_dt' ");
         ...
         if($count_archive==$count)
         {
           /* echo "Successfully Inserted into archive table. Record Success Date:- ".$record_dt;*/
            ...
            $sql_del="delete FROM $smartcutoffdetals WHERE date(created_date)='$record_dt'";
         ...
         else
         {
            ...
            $sql_del="delete FROM $monthly_archive_table WHERE date(created_date)='$record_dt'";
            $result_del=mysqli_query($dbc,$sql_del);
       ...
       $myfile = fopen("/var/www/html/itswe_panel/controller/logfiles/remove_smartcutoff_record_log.txt", "a") or die("Unable to open file!");
