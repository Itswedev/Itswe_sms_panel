...
class pushsms_rcs extends common{
...
function sendRCSSMSSave($userid) {
        ...
        $count_rcs_mob=count($v_num);

        if($count_rcs_mob>10000)
        {
            return array('status' => false, 'msg' => 'mobile number limit exceed');
                                exit;
        }
...
*/
        ...
        foreach($query_data as $values) {
            ...
            if($sms_type=='dynamic')
            {
                ...
                $formData = array();
                foreach ($_REQUEST as $variable => $val) {

                    if (strpos($variable, 'var_') === 0) {
                        $newVariableName = substr($variable, 4);
                        $formData[$newVariableName] = $val;
                    }
                }
            }

        /*send sms to rbm*/
        $query_master_tbl_select= "select * from $sendtabledetals where msg_job_id='".$job_id."' and is_picked=0";
         ...
         $endpoint = 'https://api.virbm.in/rcs/v1/phones/'.$mobile_number.'/agentMessages/async?sendGipLink=false&messageId='.$master_job_id.'&botId='.$bot_id;
                ...
                'contentMessage' => array(
                    'templateMessage' => array(
                        'templateCode' => $template_name,
                         'customParams' => json_encode($formData)
            ...
            else
            {
                ...
                'contentMessage' => array(
                    'templateMessage' => array(
                        'templateCode' => $template_name
                         // 'customParams' => json_encode(array('Rates' => '0.10','Name' => 'xyz','Product' => 'Bulk SMS'))
        ...
        $filename=$job_id."_rcs_".time().".json";
        ...
        if(!empty($check_numbers))
        {
           ...
           /* print_r($headers);*/
            ...
            curl_close($curl);
             ...
             $filename=$job_id."_capability_".time().".json";
                    $file_path="classes/rcs_logs/".$filename;
                    $fp = fopen($file_path, 'a+');
                    fwrite($fp, $response);   // here it will print the array pretty
                    fclose($fp);
...
public function getBlockNumbers_json() {
       ...
       $dnd_file = "/var/www/html/controller/classes/block.json";
