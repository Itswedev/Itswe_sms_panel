...
else if(isset($_REQUEST['type']) && $_REQUEST['type'] == 'self_recharge_history')
{

   $recharge_history= loadData_self();
   echo $recharge_history;
   ...
   echo $userdata;*/
			...
			$columns = array(
			    ...
			    array( 'db' => 'userid','dt' => 2,'formatter'=>function($d,$row){
			    	$parent=array();
			    	   $parent_users=get_parentUsers($d,$parent_ids=0);
			    	   $parent_users=array_unique($parent_users);
			    	   $parent_ids=implode(",", $parent_users);
			    		 $parent_name=get_parent_username($parent_ids); 
			    	return $parent_name;
			    } ),
...
else if(isset($_REQUEST['type']) && $_REQUEST['type'] == 'all_inactive_user')
{
   ...
   echo $userdata;*/
		 ...
		 global $sql_details ;
			...
			if($userid!='1')
			{
				if($extraWhere!="")
				{
					$extraWhere=" and parent_id='$userid' and user_role!='mds_acc' and user_status='0' ";
				}
				else
				{
					$extraWhere=" parent_id='$userid' and user_role!='mds_acc' and user_status='0' ";
				}
			}
			else
			{
				$extraWhere=" user_role!='mds_acc' and user_status='0'";
			}
...
function load_overview($edit_userid)
{
	...
	if($count>0)
	{
		...
		//$record_user['user_dtls']=$record_user;
			...
			$sql_rcs="select rcs_sms_rate,rcs_rich_card_rate,a2p_rate,p2a_rate from `settings` where userid='$edit_userid'";
			$result_rcs=mysqli_query($dbc,$sql_rcs);
			$count_rcs=mysqli_num_rows($result_rcs);
			if($count_rcs>0)
			{

				while ($row_rcs=mysqli_fetch_array($result_rcs)) {
					$record_rcs['rcs_rate']=$row_rcs;
				}

				$record=array_merge($record_user,$plan_name,$route_dtls,$user_role,$record_rcs);
			...
			if($count_cutoff>0)
			{
				...
				if($count_rcs>0)
				{
					$record=array_merge($record_user,$plan_name,$route_dtls,$user_role,$record_rcs,$record_cut_off);	
				}
				else
				{
...
function get_parentUsers($userid,$parent_ids=1)
{
  global $dbc;
  // $ids = array();
  ...
  // $userids=implode(",", $userid);
       ...
       $qry = "SELECT parent_id FROM az_user WHERE userid='$userid' order by userid asc limit 1";
...
function login_to_acct()
{
	...
	if ($count > 0) {
		...
		$prev_user_name=$_SESSION['user_name'];
		$prev_client_name=$_SESSION['client_name'];
		$prev_status=$_SESSION['status'];
		$prev_user_role=$_SESSION['user_role'];
		$prev_parent_id=$_SESSION['parent_id'];
		$prev_miscall_access=$_SESSION['miscall_access'];
		$prev_vsms_access=$_SESSION['vsms_access'];
		$prev_rcs_access=$_SESSION['rcs_access'];
		$prev_acct_manager=$_SESSION['acct_manager'];
		$prev_update_dlr=$_SESSION['update_dlr'];
		$prev_spam_msg=$_SESSION['spam_msg'];
		$prev_rcs=$_SESSION['rcs'];
		$prev_lms=$_SESSION['lms'];
		$prev_vas=$_SESSION['vas'];
		$prev_intl=$_SESSION['intl'];
		$prev_voice_call=$_SESSION['voice_call'];
		$prev_campaign_report=$_SESSION['campaign_report'];
		$prev_miscall_report=$_SESSION['miscall_report'];
		$prev_gvsms=$_SESSION['gvsms'];
		$prev_BlockNum = $_SESSION['BlockNum'];
		$prev_restricted_tlv = $_SESSION['restricted_tlv'];
		$prev_api_key = $_SESSION['api_key'];
		$prev_restricted_report = $_SESSION['restricted_report'];
		$prev_profile_pic = $_SESSION['profile_pic'];
		...
		//Old account login
		$_SESSION['prev_user_id']           =             $prev_user_id ;
		$_SESSION['prev_user_name']         =             $prev_user_name ;
		$_SESSION['prev_client_name']       =             $prev_client_name ;
		$_SESSION['prev_status']            =             $prev_status  ;
		$_SESSION['prev_user_role']         =             $prev_user_role ;
		$_SESSION['prev_parent_id']         =             $prev_parent_id ;
		$_SESSION['prev_miscall_access']    =             $prev_miscall_access  ;
		$_SESSION['prev_vsms_access']       =             $prev_vsms_access ;
		$_SESSION['prev_rcs_access']        =             $prev_rcs_access  ;
		$_SESSION['prev_acct_manager']      =             $prev_acct_manager  ;
		$_SESSION['prev_rcs']               =             $prev_rcs ;
		$_SESSION['prev_lms']               =             $prev_lms ;
		$_SESSION['prev_vas']               =             $prev_vas ;
		$_SESSION['update_dlr']               =             $update_dlr ;
		$_SESSION['spam_msg']               =             $spam_msg ;
		$_SESSION['prev_voice_call']        =             $prev_voice_call  ;
		$_SESSION['prev_campaign_report']   =             $prev_campaign_report ;
		$_SESSION['prev_miscall_report']    =             $prev_miscall_report  ;
		$_SESSION['prev_gvsms']             =             $prev_gvsms ;
		$_SESSION['prev_BlockNum']          =             $prev_BlockNum  ;
		$_SESSION['prev_restricted_tlv']    =             $prev_restricted_tlv  ;
		$_SESSION['prev_api_key']           =             $prev_api_key   ;
		$_SESSION['prev_restricted_report'] =             $prev_restricted_report   ;
		$_SESSION['prev_profile_pic']       =             $prev_profile_pic   ;




            $login_id=$row_login['userid'];
            /*$sql_login_update="update az_user set login=1,logout=0 where userid='$login_id'";
            $rs_login_update=mysqli_query($dbc,$sql_login_updatel);*/
            if($login_id!=1)
            {
               $sql_count_roles="select count(user_role) as count_roles,user_role from az_user where parent_id=$login_id and (user_role='mds_usr' ||user_role='mds_sub_usr' || user_role='mds_ad' || user_role='mds_rs') group by user_role order by user_role";
            ...
            {
               $sql_count_roles="select count(user_role) as count_roles,user_role from az_user where (user_role='mds_usr' || user_role='mds_ad' || user_role='mds_rs' || user_role='mds_supp') group by user_role order by user_role";
               $rs_count_roles=mysqli_query($dbc,$sql_count_roles);
               while($row_count_roles=mysqli_fetch_array($rs_count_roles))
               {
                  $roles[]=$row_count_roles['user_role'];
...
function loadData()
{
  ...
  foreach ($child_users as $child_val) {
                     ...
                     $single_arr=array_unique($single_arr);
                      $userids=implode(",", $single_arr);

	$user_role=$_SESSION['user_role'];
	if($user_role=='mds_adm')
	{
		$sql="select c_dtls.assign_credit as a_credit,c_dtls.userid as userid,c_mng.az_routeid as routeid,c_dtls.balance as d_balance,c_dtls.remark as rmk ,c_dtls.debit_credit as dc_type,c_dtls.credit_date as c_dt from az_credit_details as c_dtls join az_credit_manage as c_mng on c_dtls.crmdid=c_mng.crmid  order by c_dt desc";
	}
	else
	{
		if(!empty($userids) and $user_role!='mds_usr')
		{

				$userid=$_SESSION['user_id'];
				

				$sql="select c_dtls.assign_credit as a_credit,c_dtls.userid as userid,c_mng.az_routeid as routeid,c_dtls.balance as d_balance,c_dtls.remark as rmk ,c_dtls.debit_credit as dc_type,c_dtls.credit_date as c_dt from az_credit_details as c_dtls join az_credit_manage as c_mng on c_dtls.crmdid=c_mng.crmid where c_dtls.userid in ($userids) order by c_dtls.created desc";
		}
		else
		{
				$userid=$_SESSION['user_id'];
	...
	if($count>0)
	{
		...
		while($row=mysqli_fetch_array($result))
		{
			...
			$data.="<tr>
				<td width='10%'> ".date('Y-m-d h:i:s a', strtotime($row['c_dt']))."</td>
...
*/
	$user_role=$_SESSION['user_role'];
		...
		if($user_role!='mds_usr')
		{
				...
				$sql="select c_dtls.assign_credit as a_credit,c_dtls.userid as userid,c_mng.az_routeid as routeid,c_dtls.balance as d_balance,c_dtls.remark as rmk ,c_dtls.debit_credit as dc_type,c_dtls.credit_date as c_dt from az_credit_details as c_dtls join az_credit_manage as c_mng on c_dtls.crmdid=c_mng.crmid where c_dtls.userid = $userid order by c_dtls.created desc";
		}
...
function loadUserData()
{
	...
	if($role!='mds_adm')
	{
		$sql="select * from az_user where parent_id='$userid' and user_status=1";
	}
	else
	{
		$sql="select * from az_user where user_status=1";
	}
...
function get_parent_username($uid)
{
	global $dbc;
	$sql="select * from az_user where userid in ($uid)";
	$uname="";
	$result=mysqli_query($dbc,$sql);
	while ($row=mysqli_fetch_array($result)) {
		$uname.=$row['user_name']." - > ";
