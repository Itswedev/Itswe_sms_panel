...
def send_master_tbl(message_id,job_id):
        ...
        master_tbl_qry = "SELECT id,mobile_number,senderid,msgcredit,unicode_type,userids,service_id,is_scheduled,request_code,send_msg,metadata,route FROM " + sendtabledetals + " WHERE is_picked=0 AND (is_scheduled=0) and message_id='"+str(message_id)+"' and msg_job_id='"+str(job_id)+"' and cut_off='No' and `status`='Submitted' ORDER BY id DESC LIMIT 2000"
        ...
        if send_count_count > 0 :
            ...
            if route=='OTP':
                send_sms_qry = "INSERT INTO `otp_send` (`momt`, `sender`, `receiver`, `msgdata`, `smsc_id`,`sms_type`, `coding`, `dlr_mask`, `dlr_url`, `charset`, `boxc_id`, `udh`,`meta_data`) values "+insert_send_sms_val
            else:
                send_sms_qry = "INSERT INTO `send_sms` (`momt`, `sender`, `receiver`, `msgdata`, `smsc_id`,`sms_type`, `coding`, `dlr_mask`, `dlr_url`, `charset`, `boxc_id`, `udh`,`meta_data`) values "+insert_send_sms_val
            # print(send_sms_qry)
