...
error_reporting(0);
class pushsms_voice extends common{
    ...
    function sendQuickCALLSave($userid) {
        ...
        if ($_POST['voice_file'] == '' || $_POST['numbers'] == '' ) {
         ...
         $sql_fetch_multimedia="select get_response,file_duration,voice_id,sender_id,template_id from multimedia where id='".$multimedia_id."'";
    ...
    /*return array('status' => false, 'rs' => $rId,'job_id' => $job_id,'qry' => $q);
            ...
            foreach ($v_num as $value) 
            {
                    ...
                    //$service_name='LIVE';
                     if(!empty($dnd_num))
                        ...
                        $cut_off_status="No";
                        $str[]= '(NULL, "' . $rId . '", "'.$num.'", NOW(), "' . $is_schedule . '", "' . $schdate . '", "' . $msgcredit . '", "' . $userids . '","' . $status . '", "' . $err_code . '",  "'.$route_name.'","'.$job_id.'", "'.$schedule_sent.'", "'.$parent_id.'", "'.$sent_at.'","'.$cut_off_status.'","'.$master_job_id.'","'.$multimedia_id.'","'.$file_name.'","'.$caller_id.'","'.$trans_id.'","'.$retry_duration.'")';
        ...
        if($_POST['is_schedule'] != '1')
        {
            ...
            copy("/var/www/html/itswe_panel/controller/classes/sent_call.php", "/var/www/html/itswe_panel/controller/classes/sent_call/".$new_send_file);
            /*return array('status' => false, 'rs' => $rId,'job_id' => $job_id,'qry' => $q);
            exit;*/
            exec("php /var/www/html/itswe_panel/controller/classes/sent_call/$new_send_file $filename > /dev/null 2>/dev/null &"); 
        ...
        else
        {
                    ...
                    foreach($query_data as $values) {
                        ...
                        /*  $msg_arr[]=$values;*/
                        $query_master_tbl_insert= "INSERT INTO $sendtabledetals (`id`, `message_id`,  `mobile_number`, `created_at`,  `is_scheduled`, `scheduled_time`, `msgcredit`, `userid`,  `status`, `err_code`,`route`,`msg_job_id`,`schedule_sent`,`parent_id`,`sent_at`,`cut_off`,`master_job_id`,`multimedia_id`,`file_name`,`caller_id`,`trans_id`,`retry_duration`) VALUES $insert_val";
                        $rs1 = mysqli_query($dbc, $query_master_tbl_insert) or die(mysqli_error($dbc));


                        }

                    $new_send_file="run_voice_schedule_sms".time().".php";
                    copy("/var/www/html/itswe_panel/controller/classes/run_voice_schedule_sms.php", "/var/www/html/itswe_panel/controller/classes/schedule_voice/".$new_send_file);
...
function fetch_sender_name($sid)
{
    ...
    function vsms($num_count,$msg)
    {
       ...
       $pyout = exec("/var/www/html/itswe_panel/python-venv/bin/python /var/www/html/itswe_panel/controller/vsms/create_hashes_example.py $filename $user_id >> /var/www/html/itswe_panel/controller/vsms/test_vsms.log");
        return $pyout;
    }
    ...
    function vsms_dynamic($num_count,$msg)
    {
       ...
       $pyout = exec("/var/www/html/itswe_panel/python-venv/bin/python /var/www/html/itswe_panel/controller/vsms/create_hashes_example1.py $filename $user_id >> /var/www/html/itswe_panel/controller/vsms/test_vsms.log");
        return $pyout;
    }
    ...
    function send_rcs_msg($num_count,$msg,$message_type)
    {
        ...
        if($message_type=='text')
        {
         ...
         $pyout = exec("/var/www/html/itswe_panel/python-venv/bin/python /var/www/html/itswe_panel/rcs/send_simple_text.py rcs_send_sms1.json", $outp, $return);
        }
        else if($message_type=='standalone' || $message_type=='carousel')
        {
        $pyout = exec("/var/www/html/itswe_panel/python-venv/bin/python /var/www/html/itswe_panel/rcs/rich_card.py rcs_send_sms1.json", $outp, $return);
        }
        return $pyout;
