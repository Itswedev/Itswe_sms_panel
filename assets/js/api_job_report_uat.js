...
function load_send_jobs_report()
{
	...
	var list_type="api_job_table_dtls_uat"; 
	//var full_url = window.location.origin;
...
async function waitForNextPageToLoad() {
    return new Promise((resolve) => {
        const observer = new MutationObserver((mutations, me) => {
            // Assume that new rows indicate page change, adjust if necessary
        ...
        // Observe changes to the entire document or a more specific container
...
async function downloadTableAsCSV(tableId, filename) {
    let tableData = [];
    let hasNextPage = true;

    // Function to get the "Next" button
    const getNextPageButton = () => document.querySelector(".next-page-button"); // Change this selector to match your actual pagination control

    while (hasNextPage) {
        // Extract and append current page data
        let currentPageData = await extractTableData(tableId);
        tableData = tableData.concat(currentPageData);
        ...
        if (nextPageButton && !nextPageButton.disabled) {
            ...
            await waitForNextPageToLoad(); // Wait for the next page to load
        } else {
            hasNextPage = false;
        }
    }
